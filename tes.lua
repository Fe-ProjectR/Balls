-- ╔════════════════════════════════════════════════════════════════════════════════╗
-- ║ NETFLIX THEMED GUI MENU v2 - LocalScript ║
...
-- 🔍 CUSTOM SEARCH BAR ICON (Replace with your GitHub PNG image link!)
local SEARCH_ICON_URL = "https://raw.githubusercontent.com/Fe-ProjectR/GAME-IMAGES/refs/heads/main/Untitled226_20260710163154.png"
-- ═══════════════════════════════════════════════════════════════════════════════
-- 🎨 THEME
-- ═══════════════════════════════════════════════════════════════════════════════
local COLORS = {
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 🌐 BULLETPROOF EXTERNAL IMAGE LOADER
-- ═══════════════════════════════════════════════════════════════════════════════
local function getExternalImage(url)
if not url or type(url) ~= "string" or url == "" then return "" end
-- If it's already a standard roblox asset, just return it
if url:find("rbxassetid://") then return url end

-- Check for executor functions safely
local getasset = getcustomasset or getsynasset
if not getasset then return "" end

-- Generate a safe, unique filename based on the URL
local safeName = url:gsub("[^%w%._]", "")
safeName = safeName:sub(-30) 
if safeName == "" then safeName = tostring(math.random(1000, 9999)) end
local fileName = "NetflixGUI_" .. safeName .. ".png"

-- Safely check if file already exists
local fileExists = false
pcall(function()
    if type(isfile) == "function" and isfile(fileName) then
        fileExists = true
    elseif type(readfile) == "function" and readfile(fileName) then
        fileExists = true
    end
end)
...
if fileExists then
    local assetUrl = ""
    pcall(function() assetUrl = getasset(fileName) end)
    return assetUrl or ""
end
...
-- Save it and load as custom asset
if success and result and type(writefile) == "function" then
    pcall(function()
...
-- 📦 ROOT GUI
-- ═══════════════════════════════════════════════════════════════════════════════
/* STREAMING_CHUNK:Initializing screen elements... */
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- 🎬 MENU FRAME (Search & Tabs)
-- ═══════════════════════════════════════════════════════════════════════════════
/* STREAMING_CHUNK:Styling Search box and custom GitHub search icon... */
...
-- 🎨 NEW SEARCH ICON IMAGE (GitHub Loaded)
local SearchIcon = Instance.new("ImageLabel")
SearchIcon.Name = "SearchIcon"
SearchIcon.Size = UDim2.new(0, 16, 0, 16)
SearchIcon.Position = UDim2.new(0, 8, 0.5, -8) -- Perfectly centered vertically inside 32px height bar
SearchIcon.BackgroundTransparency = 1
SearchIcon.ScaleType = Enum.ScaleType.Fit
SearchIcon.Parent = SearchFrame
-- Load search icon image asynchronously in background
task.spawn(function()
local assetUrl = getExternalImage(SEARCH_ICON_URL)
if assetUrl ~= "" then
SearchIcon.Image = assetUrl
end
end)
-- Shift TextBox over slightly to prevent text overlapping the new icon image
...
-- 🎠 CAROUSEL
-- ═══════════════════════════════════════════════════════════════════════════════
/* STREAMING_CHUNK:Structuring carousel layout details... */
...
-- 🎬 CAROUSEL LOGIC
-- ═══════════════════════════════════════════════════════════════════════════════
/* STREAMING_CHUNK:Constructing 3D card movement functions... */
local currentIndex = 1
local visibleMovies = {}
local cardWidth, cardGap = 150, 14
local cardImgHeight = cardWidth * (400/600)
...
-- ✨ ASYNC IMAGE LOADER ✨
task.spawn(function()
    local assetUrl = getExternalImage(movie.Img)
    if assetUrl ~= "" then
        img.Image = assetUrl
    end
end)
...
-- 📱 VIEW SWITCHING
-- ═══════════════════════════════════════════════════════════════════════════════
/* STREAMING_CHUNK:Configuring slide transition logic... */
...
DetailImage.Image = "" -- Clear old image
-- Load detailed image in the background
task.spawn(function()
    local assetUrl = getExternalImage(movie.Img)
    if assetUrl ~= "" then
        DetailImage.Image = assetUrl
    end
end)
...
-- 📦 MINIMIZE
-- ═══════════════════════════════════════════════════════════════════════════════
/* STREAMING_CHUNK:Finalizing layout minimize event handling... */
local minimized = false
local savedWindowPos = MainFrame.Position
local savedPillPos = nil
...
-- ✨ TOUCH/DRAG VS CLICK FIX ✨
-- This tracks if the user dragged their finger. If they moved it, it won't open the UI.
...
-- Calculate distance between initial press and release
local dist = (input.Position - pillClickStartPos).Magnitude
        -- If distance is less than 10 pixels, count it as a tap/click
        if dist < 10 then
            setMinimized(false)
        end
